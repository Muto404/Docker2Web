...
如果 DNS 正常、Mac 上访问返回 200，但容器内经 `host.docker.internal:443` 的 TLS 检查超时，可以让 Docker2Web 加入现有 NPM 网络：
...
在连接设置中将「NPM 代理地址」改为 NPM 容器名（例如 `npm-npm-1`），端口仍为 443。管理地址和业务转发主机不需要随之改变。要让后续普通 `docker compose up -d` 保留网络覆盖，可在本地 `.env` 增加 `COMPOSE_FILE=compose.yaml:compose.npm-network.yaml`（macOS/Linux）。外部 NPM 网络应先存在。Docker2Web 的 Docker reader 仍然只加入内部 reader 网络。
